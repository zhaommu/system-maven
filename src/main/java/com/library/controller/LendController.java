...
public class LendController {
    ...
    public String deleteBook(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        if (bookService.deleteBook(bookId)) {
            redirectAttributes.addFlashAttribute("succ", "商标删除成功！");
        } else {
            redirectAttributes.addFlashAttribute("error", "商标删除失败！");
        }
    ...
    public String bookLend(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        if (lendService.lendBook(bookId, readerId)) {
            redirectAttributes.addFlashAttribute("succ", "商标借阅成功！");
        } else {
            redirectAttributes.addFlashAttribute("succ", "商标借阅成功！");
        }
    ...
    public String bookReturn(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        if (lendService.returnBook(bookId, readerId)) {
            redirectAttributes.addFlashAttribute("succ", "商标归还成功！");
        } else {
            redirectAttributes.addFlashAttribute("error", "商标归还失败！");
        }
        return "redirect:/reader_books.html";
    }
}
